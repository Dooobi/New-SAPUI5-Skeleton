...
module.exports = {
	...
	checkAuthData: function(authData) {
		if (authData && authData.cookieString && authData.clientSessionId 
			&& !authData.invalidated && authData.createdOn && authData.validTo) {
	...
	getAuthData: async function(username, password) {
		try {
			var authData = this.getAuthDataFromFile();
	...
	importApp: async function(username, password, account, appname, pathToZip, version) {
		try {
			...
			var fileContent = fs.readFileSync(pathToZip, { encoding: "binary" });
			fileContent = iconv.decode(fileContent, "ISO-8859-1");
